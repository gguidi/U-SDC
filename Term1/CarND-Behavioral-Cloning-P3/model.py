	...
	while 1: # Loop forever so the generator never terminates
		...
		for offset in range(0, num_samples, batch_size):
			...
			if center_image_only:
				X_train = np.empty([batch_size*2,resized_height,resized_width,3], dtype=np.float_)
				y_train = np.empty([batch_size*2], dtype=np.float_)
			else:
				X_train = np.empty([batch_size*4,resized_height,resized_width,3], dtype=np.float_)
...
img_input_shape = (resized_height,resized_width,3)
#kernel 1 parameters
k1_row = 5
k1_column = 5
stride1 = (2,2)
...
model.add(Cropping2D(cropping=((25,10), (0,0)), input_shape=img_input_shape))
